...
async def fetch_google_satellite(lat: float, lon: float, zoom: int = 18) -> bytes | None:
    """Google Maps Static API — en güncel uydu görüntüsü."""
...
async def fetch_satellite_image(lat: float, lon: float) -> bytes | None:
    ...
    Öncelik: Google Maps → Mapbox → Esri → Sentinel Hub"""

    # 1. Google Maps Static API — en güncel, key varsa
    if GOOGLE_MAPS_KEY:
        img = await fetch_google_satellite(lat, lon, zoom=18)
        if img:
            return img

    # 2. Mapbox Satellite — alternatif güncel kaynak
    if MAPBOX_TOKEN:
        img = await fetch_mapbox_satellite(lat, lon, zoom=17)
        if img:
            return img

    # 3. Esri World Imagery — ücretsiz fallback
    img = await fetch_esri_satellite_tile(lat, lon, zoom=18)
    if img:
        return img

    # 4. Sentinel Hub (credentials varsa)
